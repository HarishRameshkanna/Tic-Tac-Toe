...
class MPConnectionManager: NSObject, ObservableObject {
    ...
    @Published var recieveInvite: Bool = false
    @Published var revievedInviteFrom: MCPeerID?
    @Published var invitiationHandler: ((Bool, MCSession?) -> Void)?
...
extension MPConnectionManager: MCNearbyServiceAdvertiserDelegate{
    ...
    @escaping (Bool, MCSession?) -> Void) {
        DispatchQueue.main.async {
            self.recieveInvite = true
            self.revievedInviteFrom = peerID
            self.invitiationHandler = invitationHandler
        }
    }
